...
# PCA plots
df_cbe = rna_normalized$CBE$edata %>%
        t() %>%
        as.data.frame()
df_diag_cbe = cbind(df_cbe, diagnosis = rna_normalized$CBE$pdata$Diagnosis)
...
df_tcx = rna_normalized$TCX$edata %>%
        t() %>%
        as.data.frame()
df_diag_tcx = cbind(df_tcx, diagnosis = rna_normalized$TCX$pdata$Diagnosis)
