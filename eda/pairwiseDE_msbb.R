...
for (region in names(rna)) {
    ...
    # Design the model
    design <- model.matrix(~diagnosis+batch, data = rna[[region]]$pdata)
    batch_levs <- length(unique(rna[[region]]$pdata$batch))
    colnames(design) <- c("NCI", "Transition", "AD", paste0("batch", 1:(batch_levs-1)))
