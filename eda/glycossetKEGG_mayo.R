...
for (region in names(DE)) {
    ...
    addDataFrame(bm$PA_glyco_sig_exclu[,7:10], sheet=sheet, startColumn=ncol(bm$AD_glyco_sig_exclu)+2, row.names=FALSE)
    addDataFrame(bm$PSP_glyco_sig_exclu[,7:10], sheet=sheet, startColumn=ncol(bm$AD_glyco_sig_exclu)+2+4, row.names=FALSE)
    ...
    addDataFrame(bm$PA_glyco_sig[,7:10], sheet=sheet, startColumn=ncol(bm$AD_glyco_sig)+2, row.names=FALSE)
    addDataFrame(bm$PSP_glyco_sig[,7:10], sheet=sheet, startColumn=ncol(bm$AD_glyco_sig)+2+4, row.names=FALSE)
    ...
    addDataFrame(bm$PA_glyco[,7:10], sheet=sheet, startColumn=ncol(bm$AD_glyco_sig_exclu)+2, row.names=FALSE)
    addDataFrame(bm$PSP_glyco[,7:10], sheet=sheet, startColumn=ncol(bm$AD_glyco_sig_exclu)+2+4, row.names=FALSE)
    addDataFrame(bm$anova_glyco[,7:9], sheet=sheet, startColumn=ncol(bm$AD_glyco_sig_exclu)+2+4+4, row.names=FALSE)
    saveWorkbook(wb, paste0("../output/mayo_modified_", region, ".xlsx"))
}
